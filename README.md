...
#### Модульные
Для запуска тестов использовала Mocha. Для заглушек - Sinon, для заглушки Express - пакет sinon-express-mock. 

##### Сценарии:
```test-controllers.js```
* в контроллерах вызывается метод render с нужными параметрами, если запрос с git CLI выполнен успешно
* в контроллерах выхывается метод next(err) с ошибкой, если обращение к git выполнено с ошибкой
